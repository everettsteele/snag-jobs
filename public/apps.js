// HopeSpot apps.js v7.0 — Dashboard, Applications, Job Board, Networking
...
// ================================================================
// RICH COMBINED DASHBOARD (overrides index.html's renderDashboard)
// ================================================================
async function renderDashboard() {
  ...
  if (!appsWk >= 5) insights.push({ text: `Only ${appsWk} app${appsWk!==1?'s':''} this week vs 5 target`, color:'#F59E0B', tab:'applications' });
  ...
  // Email outreach card
  html += `<div class="dash-card"><div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:12px"><div class="dash-card-title" style="margin:0">Email Outreach</div>${slaChip(sla.onTrack, sla.dailyAvg7+'/day')}</div>`;
  html += `<div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:8px">${kpi(totals.contacted,'Total Sent','#1F2D3D',tw(sentTrend))}${kpi(overallRate+'%','Reply Rate',overallRate>5?'#10B981':'#6B7280')}${kpi(totals.inConversation,'In Convo','#3B82F6')}${kpi(DUE.length,'Due Today',DUE.length>3?'#EF4444':'#9CA3AF')}</div>`;
  html += `<div style="font-size:11px;color:#6B7280;padding-top:6px;border-top:1px solid #F3F4F6">This week: <strong>${thisWkSent}</strong> sent &nbsp;&middot;&nbsp; <strong>${totals.drafts}</strong> drafted</div></div>`;

  // Applications card
  html += `<div class="dash-card"><div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:12px"><div class="dash-card-title" style="margin:0">Applications</div>${slaChip(appsWk>=5, appsWk+' this wk')}</div>`;
  html += `<div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:8px">${kpi(aQ+aA,'Pipeline',aQ>0?'#7c3aed':'#6b7280',tw(appTrend))}${kpi(aRR+'%','Response',aRR>15?'#10B981':'#6B7280')}${kpi(aI,'Interview','#d97706')}${kpi(aNP,'Needs Pkg',aNP>0?'#EF4444':'#10B981')}</div>`;
  html += `<div style="font-size:11px;color:#6B7280;padding-top:6px;border-top:1px solid #F3F4F6">${apps.length} tracked &nbsp;&middot;&nbsp; <strong>${aO}</strong> offer${aO!==1?'s':''} &nbsp;&middot;&nbsp; <strong>${aR}</strong> rejected</div></div>`;

  // Networking card
  html += `<div class="dash-card"><div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:12px"><div class="dash-card-title" style="margin:0">Networking</div>${slaChip(netWk>=2, netWk+' this wk')}</div>`;
  html += `<div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:8px">${kpi(netMo,'30-Day Events','#1F2D3D',tw(netTrend))}${kpi(netContacts,'Contacts','#6B7280')}${kpi(overSteps,'Overdue',overSteps>0?'#EF4444':'#10B981')}${kpi(noNotes,'No Notes',noNotes>0?'#F59E0B':'#10B981')}</div>`;
  html += `<div style="font-size:11px;color:#6B7280;padding-top:6px;border-top:1px solid #F3F4F6">${net.length} events total &nbsp;&middot;&nbsp; <strong>${pendSteps}</strong> pending steps</div></div>`;

  html += '</div>'; // end three cards

  // Middle row: chart + funnel + recent events
  ...
  fSteps.forEach(([l,v,c]) => {
    ...
    const convPct = aSubmit>0 && l!=='Queued' ? `<span style="color:#9CA3AF;font-weight:400;font-size:10px"> (${Math.round(v/aSubmit*100)}%)</span>` : '';
    html += `<div style="margin-bottom:7px"><div style="display:flex;justify-content:space-between;font-size:11px;margin-bottom:3px"><span style="color:#6B7280;font-weight:500">${l}</span><span style="color:${c};font-weight:700">${v}${convPct}</span></div><div style="height:6px;background:#F3F4F6;border-radius:3px"><div style="height:100%;background:${c};border-radius:3px;width:${pct}%"></div></div></div>`;
  });
  html += '</div>';
  // Recent events
  ...
  html += '</div></div>'; // end right, end middle grid
  ...
  // SLA panel (all three) + Template A/B
...
function renderApplications() {
  ...
  const rows = _appsData.map(app => {
    ...
    const hasPackage = app.drive_url;
    return `<tr style="border-bottom:1px solid #F3F4F6">
      <td style="padding:10px 0;font-weight:600;font-size:13px">${app.company}${app.status==='queued'&&!hasPackage?` <span style="font-size:9px;background:#FEF2F2;color:#EF4444;padding:1px 5px;border-radius:3px;vertical-align:middle">NO PKG</span>`:''}</td>
  ...
  document.getElementById('main-content').innerHTML =
    `<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:16px">
      <div><div style="font-size:22px;font-weight:700">Applications</div>
      <div style="font-size:13px;color:#9ca3af;margin-top:2px">${_appsData.length} tracked &nbsp;&middot;&nbsp; ${_appsData.filter(a=>a.status==='queued'&&!a.drive_url).length} need a package</div></div>
    ...
    <thead><tr style="border-bottom:2px solid #E5E7EB">
      <th style="text-align:left;padding:8px 0;font-size:10px;color:#9CA3AF;font-weight:700;text-transform:uppercase;letter-spacing:.05em">Company</th>
      <th style="text-align:left;padding:8px;font-size:10px;color:#9CA3AF;font-weight:700;text-transform:uppercase;letter-spacing:.05em">Role</th>
      <th style="text-align:left;padding:8px;font-size:10px;color:#9CA3AF;font-weight:700;text-transform:uppercase;letter-spacing:.05em">Added</th>
      <th style="text-align:left;padding:8px;font-size:10px;color:#9CA3AF;font-weight:700;text-transform:uppercase;letter-spacing:.05em">Status</th>
      <th style="text-align:left;padding:8px;font-size:10px;color:#9CA3AF;font-weight:700;text-transform:uppercase;letter-spacing:.05em">Follow-up</th>
      <th style="text-align:left;padding:8px;font-size:10px;color:#9CA3AF;font-weight:700;text-transform:uppercase;letter-spacing:.05em">Activity</th>
      <th style="padding:8px 0"></th>
    </tr></thead>
    <tbody>${rows||'<tr><td colspan="7" style="text-align:center;padding:32px;color:#9CA3AF">No applications yet.</td></tr>'}</tbody>
    </table></div>`;
}
  ...
  const row = (l) => {
    ...
    return `<tr style="border-bottom:1px solid #f3f4f6">
      <td style="padding:10px 14px">
        ...
        <div style="font-size:11px;color:#6b7280;margin-top:2px">${l.organization||''}${l.location?' \u00b7 '+l.location:''}</div>
        <span style="display:inline-block;margin-top:4px;padding:1px 6px;background:${sc}15;color:${sc};border-radius:4px;font-size:10px;font-weight:700">${l.source_label||l.source}</span>
      </td>
      <td style="padding:10px 14px;text-align:center"><span style="font-size:13px;font-weight:700;color:${l.fit_score>=7?'#16a34a':l.fit_score>=5?'#d97706':'#6b7280'}">${l.fit_score}/10</span></td>
      ...
      <td style="padding:10px 14px;white-space:nowrap">
        <a href="${l.url}" target="_blank" style="display:inline-block;padding:3px 9px;background:#1f2d3d;border-radius:5px;font-size:11px;color:#fff;text-decoration:none;margin-right:4px">View</a>
        ${l.status==='new'?`<button onclick="snagLead('${l.id}',this)" style="padding:3px 9px;background:#f97316;border:none;border-radius:5px;font-size:11px;color:#fff;cursor:pointer;margin-right:4px;font-weight:600">Snag</button><button onclick="updateLeadStatus('${l.id}','reviewed')" style="padding:3px 7px;background:#f3f4f6;border:none;border-radius:5px;font-size:11px;color:#374151;cursor:pointer">Skip</button>`:''}
      </td></tr>`;
  };
  document.getElementById('main-content').innerHTML =
    `<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:12px">
      <div><div style="font-size:22px;font-weight:700">Job Board</div>
      <div style="font-size:13px;color:#9ca3af;margin-top:2px">JewishJobs &middot; ExecThread &middot; CoS Network &middot; Idealist &middot; Built In ATL &middot; Daily 6 AM</div></div>
    ...
    ${newLeads.length>0
      ? `<div style="font-size:12px;font-weight:700;text-transform:uppercase;letter-spacing:.05em;color:#374151;margin-bottom:8px">New (${newLeads.length}) &mdash; Snag to add to Applications</div>
         ...
         <tbody>${newLeads.map(row).join('')}</tbody></table></div>`
    ...
    ${reviewed.length>0
      ? `<div style="font-size:12px;font-weight:700;text-transform:uppercase;letter-spacing:.05em;color:#9ca3af;margin-bottom:8px">Skipped (${reviewed.length})</div>
         <div style="background:#fff;border:1px solid #e5e7eb;border-radius:10px;overflow:hidden;opacity:.5">
         <table style="width:100%;border-collapse:collapse"><tbody>${reviewed.slice(0,8).map(row).join('')}</tbody></table></div>`
      : ''}`;
...
async function renderNetworking() {
  ...
  const totalOverdue = _netData.reduce((n,e) => n + (e.next_steps||[]).filter(ns=>!ns.done&&ns.due_date&&ns.due_date<=today).length, 0);
      ...
      <div style="display:flex;align-items:flex-start;gap:10px;margin-bottom:12px">
        <div style="flex:1">
          <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap">
            ...
            <span style="font-size:10px;font-weight:700;padding:2px 7px;border-radius:4px;background:${c}15;color:${c}">${TYPE_LABEL[e.type]||e.type}</span>
            ${future?'<span style="font-size:10px;font-weight:700;padding:2px 7px;border-radius:4px;background:#EFF6FF;color:#3B82F6">Upcoming</span>':''}
          </div>
        ...
        <div style="display:flex;gap:6px;align-items:center">
          ${ovrS>0?`<span style="font-size:10px;font-weight:700;color:#EF4444;background:#FEF2F2;padding:2px 7px;border-radius:4px">${ovrS} overdue</span>`:''}
          ${pendS>0&&ovrS===0?`<span style="font-size:10px;color:#d97706;background:#FEF3C7;padding:2px 7px;border-radius:4px">${pendS} pending</span>`:''}
  ...
  const upcoming = _netData.filter(e=>e.start_date>today);
  const past = _netData.filter(e=>e.start_date<=today);

  document.getElementById('main-content').innerHTML =
    `<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:16px">
      <div><div style="font-size:22px;font-weight:700">Events & Meetings</div>
      <div style="font-size:13px;color:#9ca3af;margin-top:2px">${_netData.length} logged &nbsp;&middot;&nbsp; ${totalOverdue} overdue steps</div></div>
    ...
    <p style="font-size:12px;color:#9CA3AF;margin-bottom:16px">Sync Google Calendar during morning sync to import upcoming meetings automatically.</p>
    ${upcoming.length>0?`<div style="font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:.06em;color:#3B82F6;margin-bottom:8px">Upcoming (${upcoming.length})</div>${upcoming.map(renderEvent).join('')}`:''}
    ${past.length>0?`<div style="font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:.06em;color:#374151;margin-bottom:8px;margin-top:${upcoming.length>0?'20px':'0'}">Past (${past.length})</div>${past.map(renderEvent).join('')}`:''}
    ${_netData.length===0?'<div style="text-align:center;padding:60px;color:#9CA3AF;background:#fff;border:1px solid #E5E7EB;border-radius:10px">No events yet. Log one manually or run morning sync to import from Google Calendar.</div>':''}`;
}

async function renderNetworkingContacts() {
  try { _netData = await (await fetch('/api/networking/events', { headers: _authFH() })).json(); } catch(e) { _netData = []; }
  // Derive contacts from events, deduplicated by email
  const contactMap = {};
  _netData.forEach(e => {
  ...
  const rows = contacts.map(c => `<tr style="border-bottom:1px solid #F3F4F6">
    <td style="padding:10px 0;font-weight:600;font-size:13px">${c.name}</td>
    <td style="padding:10px 8px;font-size:12px;color:#6B7280">${c.company||''}</td>
    <td style="padding:10px 8px;font-size:12px;color:#9CA3AF">${c.role||''}</td>
    <td style="padding:10px 8px;font-family:monospace;font-size:11px;color:#F97316">${c.email||''}</td>
    <td style="padding:10px 8px;font-size:11px;color:#9CA3AF">${c.latestDate}</td>
    <td style="padding:10px 8px;font-size:11px;color:#6B7280">${c.events.map(ev=>ev.title).join(', ')}</td>
  </tr>`).join('');
  document.getElementById('main-content').innerHTML =
    ...
    ${contacts.length===0
      ? '<div style="text-align:center;padding:60px;color:#9CA3AF;background:#fff;border:1px solid #E5E7EB;border-radius:10px">No contacts yet. Add contacts to events to see them here.</div>'
      : `<div style="overflow-x:auto"><table style="width:100%;border-collapse:collapse">
         <thead><tr style="border-bottom:2px solid #E5E7EB">
           <th style="text-align:left;padding:8px 0;font-size:10px;color:#9CA3AF;font-weight:700;text-transform:uppercase;letter-spacing:.05em">Name</th>
           <th style="text-align:left;padding:8px;font-size:10px;color:#9CA3AF;font-weight:700;text-transform:uppercase;letter-spacing:.05em">Company</th>
           <th style="text-align:left;padding:8px;font-size:10px;color:#9CA3AF;font-weight:700;text-transform:uppercase;letter-spacing:.05em">Role</th>
           <th style="text-align:left;padding:8px;font-size:10px;color:#9CA3AF;font-weight:700;text-transform:uppercase;letter-spacing:.05em">Email</th>
           <th style="text-align:left;padding:8px;font-size:10px;color:#9CA3AF;font-weight:700;text-transform:uppercase;letter-spacing:.05em">Met</th>
           <th style="text-align:left;padding:8px;font-size:10px;color:#9CA3AF;font-weight:700;text-transform:uppercase;letter-spacing:.05em">Events</th>
         </tr></thead>
...
(function inject() {
  function go() {
    ...
    // Initialize badge for job board
