...
export function AuthProvider({ children }) {
  ...
  const checkAuth = useCallback(async () => {
    // One-time migration from old hopespot_token key
    const legacyToken = localStorage.getItem('hopespot_token');
    if (legacyToken && !localStorage.getItem('snag_token')) {
      localStorage.setItem('snag_token', legacyToken);
  ...
  const login = async (email, password) => {
    ...
    localStorage.setItem('snag_token', data.token);
    setUser(data.user);
